...
class RandomForestClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            # Ambil sampel bootstrap dari data
            bootstrap_indices = np.random.choice(len(X), size=len(X), replace=True)
            X_bootstrap = X[bootstrap_indices]
            y_bootstrap = y[bootstrap_indices]

            # Buat pohon keputusan dengan parameter yang diberikan
    ...
    def _build_tree(self, X, y, depth=0):
        ...
        # Cari fitur terbaik untuk pemisahan
        best_feature, best_threshold = self._find_best_split(X, y)

        # Buat simpul kiri dan kanan
    ...
    def _calculate_information_gain(self, X, y, feature, threshold):
        # Implementasi perhitungan information gain
        pass

    def predict(self, X):
        predictions = []
        for tree in self.trees:
            predictions.append(self._predict_single_tree(tree, X))
        return np.array(predictions).T.mode(axis=1)
